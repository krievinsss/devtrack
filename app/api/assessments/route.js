...
export async function POST(req){
  ...
  try{const input=schema.parse(await req.json()),project=await getProject(input.projectId);if(!project||project.studentId!==input.studentId||!await canAccessStudent(auth.user,input.studentId))return fail('Forbidden',403);const item=await saveAssessment(input,auth.user);after(()=>processAssessmentSideEffects(item));return ok({item});}catch(e){return fail(e.message,400,e?.issues)}
}
