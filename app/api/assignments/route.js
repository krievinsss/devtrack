...
import { createAssignment,deleteAssignment,getAssignments,updateAssignment } from '@/services/assignments';
...
export async function PATCH(req){const auth=await requireApiUser(['teacher','admin']);if(auth.error)return auth.error;try{const raw=await req.json();const id=z.string().parse(raw.id);return ok({assignment:await updateAssignment(id,clean(base.parse(raw)),auth.user)});}catch(e){return fail(e.message,400,e?.issues)}}
