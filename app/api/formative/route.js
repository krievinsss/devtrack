...
import {
  createFormative,
  getFormativeEvent,
  processFormativeSideEffects,
  saveFormativeResult,
} from "@/services/formative";
...
export async function POST(req) {
  ...
  try {
    ...
    if (body.action === "create") {
      ...
      const event = await createFormative(input, auth.user);
      after(() =>
        notifyAssignmentStudents(input.assignmentId, {
          type: "assessment",
          title: "Jauns formatīvais vērtējums",
          message: event.title,
          href: "/projects",
        }),
      );
    ...
    if (body.action === "grade") {
      const input = gradeSchema.parse(body),
        event = await getFormativeEvent(input.eventId),
      ...
      if (
        ...
        return fail("Forbidden", 403);
      const result = await saveFormativeResult(input, auth.user);
      after(() => processFormativeSideEffects(input, result));
      return ok({ result });
    }
