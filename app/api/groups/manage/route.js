...
export async function POST(req){
 ...
 try{
  ...
  if(body.action==='removeFromGroup'){const groupId=z.string().parse(body.groupId),studentId=z.string().parse(body.studentId);let group=null,student=null;await Promise.all([updateJson('groups',[],items=>items.map(g=>g.id===groupId?(group={...g,studentIds:(g.studentIds||[]).filter(x=>x!==studentId),updatedAt:now()}):g)),updateJson('users',[],items=>items.map(u=>u.id===studentId?(student={...u,groupIds:(u.groupIds||[]).filter(x=>x!==groupId),updatedAt:now()}):u))]);return ok({group,student:student?(({passwordHash,...safe})=>safe)(student):null});}
  if(body.action==='deleteStudent'){
   ...
   await Promise.all([
    updateJson('users',[],items=>items.filter(u=>u.id!==studentId)),
    updateJson('groups',[],items=>items.map(g=>({...g,studentIds:(g.studentIds||[]).filter(x=>x!==studentId),updatedAt:now()}))),
    updateJson('projects',[],items=>items.filter(p=>p.studentId!==studentId)),
    updateJson('commits',[],items=>items.filter(c=>!projectIds.has(c.repositoryId)&&!projectIds.has(c.projectId))),
    updateJson('feedback',[],items=>items.filter(f=>f.studentId!==studentId&&!projectIds.has(f.projectId))),
    updateJson('assessments',[],items=>items.filter(a=>a.studentId!==studentId&&!projectIds.has(a.projectId))),
    updateJson('aiReviews',[],items=>items.filter(r=>r.studentId!==studentId&&!projectIds.has(r.projectId))),
    updateJson('attendance',[],items=>items.filter(a=>a.studentId!==studentId))
   ]);
