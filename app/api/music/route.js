...
import {
  ...
  getMusicQueue,
  getMusicSettings,
  moderateMusicRequest,
  payMusicFine,
  removeMusicRequest,
  setMusicRequestsEnabled,
  unbanMusicStudent
} from '@/services/music';
...
const actionSchema = z.union([
  ...
  z.object({action:z.literal('moderate_request'),requestId:z.string(),status:z.enum(['approved','queued','played','removed'])}),
...
export async function GET(){
  ...
  try{
    const [settings,queue] = await Promise.all([getMusicSettings(),getMusicQueue()]);
    const payload = {settings,queue,rules:MUSIC_RULES,rulesVersion:MUSIC_RULES_VERSION};
    if(auth.user.role === 'student'){
      payload.access = await getMusicAccess(auth.user.id);
    }else{
      const [users,access] = await Promise.all([getUsers(),getMusicAccessList()]);
      payload.students = users.filter(u=>u.role==='student').map(s=>({
        id:s.id,
        name:`${s.firstName} ${s.lastName}`,
        email:s.email,
        access:access.find(a=>a.studentId===s.id)||null
      }));
    }
    return ok(payload);
  }catch(e){return fail(e.message||'Could not load Classroom Music',500)}
}
  ...
  try{
    ...
    switch(body.action){
      case 'accept_rules':
        ...
        result = await acceptMusicRules(auth.user.id); break;
      case 'request':
        if(auth.user.role!=='student') throw new Error('Student action only');
        result = await addMusicRequest(auth.user, body); break;
      ...
      case 'pay_fine':
        if(auth.user.role!=='student') throw new Error('Student action only');
        result = await payMusicFine(auth.user.id); break;
    }
    return ok({result});
  }catch(e){return fail(e.message||'Music action failed',400,e?.issues)}
}
