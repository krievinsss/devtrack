...
import { Bell,CheckCheck,MessageSquareText,Award,ClipboardCheck } from 'lucide-react';
...
export default function NotificationsBell({user}){
  ...
  useEffect(()=>{if(user?.role!=='student')return;load();const timer=setInterval(load,2500),refresh=()=>load(),click=e=>{if(root.current&&!root.current.contains(e.target))setOpen(false)};window.addEventListener('devtrack-data-refresh',refresh);document.addEventListener('mousedown',click);return()=>{clearInterval(timer);window.removeEventListener('devtrack-data-refresh',refresh);document.removeEventListener('mousedown',click)}},[load,user?.role]);
