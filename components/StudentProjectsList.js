...
export default function StudentProjectsList({initialItems=[]}){
  ...
  useEffect(()=>{sync();const timer=setInterval(sync,1500);const focus=()=>sync();window.addEventListener('focus',focus);document.addEventListener('visibilitychange',focus);window.addEventListener('devtrack-data-refresh',focus);return()=>{clearInterval(timer);window.removeEventListener('focus',focus);document.removeEventListener('visibilitychange',focus);window.removeEventListener('devtrack-data-refresh',focus)}},[sync]);
