...
export async function readJson(name, fallback = null) {
  if (useBlob) {
    ...
    try {
      ...
      // Mutable JSON blobs must bypass the CDN; otherwise a refresh can return
      // the previous version after a successful save until the cache expires.
...
// Concurrent-safe event log. Every mutation is a new immutable object instead of
// overwriting shared JSON, which avoids lost updates in serverless deployments.
export async function appendVersionedEvent(namespace, key, event) {
  const value={...event,eventAt:event.eventAt||new Date().toISOString()};
  await writeVersionedJson(`events-${namespace}`,key,value);
  return value;
}

export async function readVersionedEvents(namespace, key) {
  if (useBlob) {
    const { list, get } = await blobApi();
    const prefix=`devtrack/versioned/events-${namespace}/${key}/`;
  ...
  try {
    const dir=path.join(DATA_DIR,'versioned',`events-${namespace}`,key);
    const names=(await fs.readdir(dir)).filter(x=>x.endsWith('.json')).sort();
