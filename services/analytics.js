...
export async function studentAnalytics(studentId){
  ...
  const teacherScores=projects.map(p=>p.teacherScore).filter(v=>Number.isFinite(v));
  const averageScore=scores.length?scores.reduce((s,v)=>s+v,0)/scores.length:teacherScores.length?teacherScores.reduce((s,v)=>s+v,0)/teacherScores.length:0;
...
export function weeklySeries(commits,weeks=12){const now=new Date();const out=[];for(let i=weeks-1;i>=0;i--){const start=new Date(now);start.setUTCDate(start.getUTCDate()-i*7-start.getUTCDay()+1);start.setUTCHours(0,0,0,0);const end=new Date(start);end.setUTCDate(end.getUTCDate()+7);out.push({label:start.toLocaleDateString('lv-LV',{day:'2-digit',month:'short'}),value:commits.filter(c=>new Date(c.timestamp)>=start&&new Date(c.timestamp)<end).length});}return out;}
