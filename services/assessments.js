...
export async function getAssessment(projectId) {
  return (
    (await readJson("assessments", [])).find(
...
export async function saveAssessment(input, user) {
  ...
  if (!project || project.studentId !== input.studentId)
    throw new Error("Student does not belong to this project");
  const existing =
    assessments.find((a) => a.projectId === input.projectId) || null;
  ...
  const grade =
    input.manualGrade ??
    (maxTotal ? gradeFromPercent(percent) : (existing?.grade ?? 1));
  const now = new Date().toISOString(),
    revisionHistory = existing
      ...
      : [];
  const item = {
    id: input.id || existing?.id || `assessment_${crypto.randomUUID()}`,
    ...
    percent,
    grade,
    manualGrade: input.manualGrade ?? existing?.manualGrade ?? null,
    revisionHistory,
    updatedAt: now,
  };
  await writeJson("assessments", [
    item,
    ...assessments.filter(
      (a) =>
        !(a.projectId === input.projectId && a.studentId === input.studentId),
    ),
  ]);
  return item;
}
...
export async function processAssessmentSideEffects(item) {
  try {
    ...
    await Promise.all([
      ...
      notifyStudent(item.studentId, {
        type: "grade",
        title: "Gala atzīme publicēta",
        message: `Atzīme ${item.grade}${item.percent != null ? ` · ${item.percent}%` : ""}`,
        href: `/projects/${item.projectId}`,
        projectId: item.projectId,
      }),
    ]);
  } catch (error) {
    console.error("Assessment side effects failed", {
