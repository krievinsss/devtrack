...
async function ensureStudentProjects(assignment){
  const [groups,users]=await Promise.all([getGroups(),getUsers()]); const group=groups.find(g=>g.id===assignment.groupId); if(!group)throw new Error('Group not found');
...
export async function createAssignment(input,user){
  const groups=await getGroups(); const group=groups.find(g=>g.id===input.groupId); if(!group)throw new Error('Group not found');
