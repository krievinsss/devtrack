...
import { xpForGrade } from '@/lib/gradeRewards';
...
function assessmentXp(items,sourceType){return items.reduce((sum,a)=>sum+xpForGrade(a.grade,sourceType),0)}
function eventXp(events,studentId,sourceType){return events.reduce((sum,event)=>sum+assessmentXp((event.results||[]).filter(result=>result.studentId===studentId),sourceType),0)}

export async function evaluateGamificationProgress(studentId){
  const [projects,commits,attendance,assessments,formativeEvents,summativeEvents,transactions,musicEvents]=await Promise.all([readJson('projects',[]),readJson('commits',[]),readJson('attendance',[]),readJson('assessments',[]),readJson('formativeAssessments',[]),readJson('summativeAssessments',[]),readJson('gamificationTransactions',[]),readVersionedEvents('music','classroom')]);
  ...
  const stackDevXp=transactions.filter(t=>t.studentId===studentId&&t.type==='stackdev_solution').reduce((sum,t)=>sum+Math.max(0,Number(t.xp||0)),0);
  const canonicalXp=assessmentXp(studentAssessments,'final')+eventXp(formativeEvents,studentId,'formative')+eventXp(summativeEvents,studentId,'summative')+stackDevXp,canonicalLevel=levelForXp(canonicalXp);
  ...
  if(xpBefore!==canonicalXp)additions.push({id:`xp_reconcile_${studentId}_${Date.now()}`,studentId,type:'xp_reconcile',credits:0,xp:canonicalXp-xpBefore,label:'XP synchronized from all assessments',createdAt:new Date().toISOString()});
