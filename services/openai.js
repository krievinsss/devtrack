...
export async function runAiReview(projectId,user,teacherPrompt=''){
  ...
  const input={teacherPrompt:(teacherPrompt||'').trim()||'Review this project carefully against the assignment rubric and provide concise, evidence-based findings for the teacher.',assignment:assignment?{title:assignment.title,description:assignment.description,requirements:assignment.requirements,rubric:assignment.rubric,deadline:assignment.deadline}:{description:project.assignment||'',rubric:[]},project:{name:project.name,technologies:project.technologies},repository:{files,commits},formativeHistory:formative.map(e=>({title:e.title,date:e.date,criteria:e.criteria,result:(e.results||[]).find(r=>r.studentId===project.studentId)||null})),teacherFeedback:feedback};
  const response=await client.responses.create({model:process.env.OPENAI_MODEL||'gpt-5.6-mini',input:[{role:'system',content:'You are a private programming-teacher assistant. Follow the teacherPrompt closely. Evaluate only against supplied assignment requirements and rubric, cite code evidence, consider Git history and formative history, and never claim to set the final grade. Your output is private to the teacher.'},{role:'user',content:JSON.stringify(input)}],text:{format:{type:'json_schema',name:'devtrack_private_review',strict:true,schema}}});
  const parsed=JSON.parse(response.output_text);const maxPoints=(assignment?.rubric||[]).reduce((s,c)=>s+Number(c.max||0),0)||25;const item={id:`review_${crypto.randomUUID()}`,projectId,createdBy:user.id,createdAt:new Date().toISOString(),maxPoints,teacherPrompt:input.teacherPrompt,...parsed};await updateJson('aiReviews',[],x=>[item,...x]);return item;
}
