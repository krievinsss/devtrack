...
export async function getSummativeEvents(assignmentId) {
  return (await readJson("summativeAssessments", []))
    .filter((x) => x.assignmentId === assignmentId)
    .sort((a, b) => new Date(a.date) - new Date(b.date));
}
export async function getSummativeEvent(id) {
  return (
    (await readJson("summativeAssessments", [])).find(
      (item) => item.id === id,
    ) || null
  );
}
...
export async function createSummative(input, user) {
  if (
    !(await readJson("assignments", [])).some(
      (a) => a.id === input.assignmentId,
    )
  )
    throw new Error("Project not found");
  const item = {
    id: `summative_${crypto.randomUUID()}`,
...
export async function saveSummativeResult(input, user) {
  ...
  const scores = (event.criteria || []).map((criterion) => ({
    ...
    score: Math.max(
      ...
      Math.min(
        Number(
          input.scores?.find((score) => score.name === criterion.name)?.score ||
            0,
        ),
  ...
  const total = scores.reduce((sum, item) => sum + item.score, 0),
    maxTotal = scores.reduce((sum, item) => sum + item.max, 0),
    percent = maxTotal ? Math.round((total / maxTotal) * 100) : 0;
  const previous = (event.results || []).find(
      (result) => result.studentId === input.studentId,
    ),
    nextGrade = gradeFromPercent(percent),
  ...
  const revisionHistory = previous
    ? [
        ...(previous.revisionHistory || []),
        {
          ...
          newGrade: nextGrade,
          correctionType: input.correctionType || "ordinary",
          changedAt: now,
          changedBy: user.id,
        },
      ]
  ...
  const saved = {
    ...
    gradedBy: user.id,
    publishedAt: now,
    eventTitle: event.title || "Summative",
...
export async function processSummativeSideEffects(input, saved) {
  if (!saved) return;
  try {
    await rewardGradedAssessment({
      studentId: input.studentId,
      sourceId: input.eventId,
      sourceType: "summative",
      grade: saved.grade,
      label: `${saved.eventTitle} · grade ${saved.grade}`,
    });
    await Promise.all([
      evaluateGamificationProgress(input.studentId),
      notifySummativeStudent(input, saved),
    ]);
  } catch (error) {
    console.error("Summative side effects failed", {
...
async function notifySummativeStudent(input, saved) {
  const project = (await readJson("projects", [])).find(
    (p) =>
      p.studentId === input.studentId && p.assignmentId === saved.assignmentId,
  );
  await notifyStudent(input.studentId, {
    type: "grade",
    title: "Summatīvais novērtēts",
    message: `${saved.eventTitle} · atzīme ${saved.grade}`,
